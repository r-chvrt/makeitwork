...
- **Scraping en tâche de fond** : chaque recherche soumise devient une « recherche suivie » ; un planificateur la rafraîchit toutes les `SCRAPE_INTERVAL_MINUTES` (défaut 30) tant qu'elle a été utilisée dans les 7 derniers jours (supprimée après 30 jours d'inactivité). La recherche interroge uniquement la base locale (réponse en ~10 ms) — seule une recherche jamais vue déclenche un scrape immédiat. Horodatage « actualisé il y a X min » affiché.
- **Pagination** : 20 offres par page, filtres et tri appliqués côté serveur sur l'ensemble des résultats
